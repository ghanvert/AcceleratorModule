...
def gather(tensor: torch.Tensor, num_processes: int = None) -> torch.Tensor:
    ...
    Args:
        ...
        num_processes (`int`, *optional*, defaults to `None`):
            Number of processes to gather the tensor to. By default, it will gather to all processes.
...
def gather_object(obj: Any, num_processes: int = None) -> list[Any]:
    ...
    Args:
        obj (`Any`):
            Object to gather.
        num_processes (`int`, *optional*, defaults to `None`):
            Number of processes to gather the object to. By default, it will gather to all processes.

    Returns:
        `list`: Gathered object.
    """
    if WORLD_SIZE == 1:
        return obj
...
def gather_into_single_process(tensor: Optional[torch.Tensor], dst: int = 0, remainder: int = 0) -> torch.Tensor:
    """
    Gather a tensor into a single process.

    Args:
        tensor (`torch.Tensor`, *optional*, defaults to `None`):
            Tensor to gather.
        dst (`int`, *optional*, defaults to `0`):
            Destination process.
        remainder (`int`, *optional*, defaults to `0`):
            Remainder to drop at the end of the tensor.
    ...
    output = None
    if rank == dst:
        rank_device = f"cuda:{dst}"  # TODO: make it dynamic for different backends
        if remainder > 0:
...
def gather_and_drop_duplicates(tensor: torch.Tensor, maximum: int) -> torch.Tensor:
    """
    Gather a tensor and drop duplicates.

    Args:
        tensor (`torch.Tensor`):
            Tensor to gather and drop duplicates from.
        maximum (`int`):
            Maximum size of the tensor.

    Returns:
        `torch.Tensor`: Gathered and dropped duplicates tensor.
    """
    tensor, padding_mask = pad(tensor, maximum, 0)
...
def all_gather_dictionary(tensors: dict[Any, torch.Tensor]) -> dict[Any, torch.Tensor]:
    """
    ...
    for key in all_keys:
        ...
        global_max_size = accelerator.gather(global_max_size).max().item()
    ...
    # gather
    padding_mask = accelerator.gather_for_metrics(padding_mask)
